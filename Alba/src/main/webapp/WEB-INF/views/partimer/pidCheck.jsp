...
<script>
   function idOk(){
      ...
      //현재의 창은 닫는다.
      opener.document.getElementById('pid').value="${pid}";
      opener.document.getElementById('sss').disabled="";
      opener.$('#pid').prop("readonly",true);
...
<h3><b>** ID 중복 확인 **</b></h3>
<form action="pidcheck" method="get">
User ID :
...
<div>
   ...
   <c:if test="${idCheck!='T'}">   
      ${pid} 는 사용 불가능한 ID 입니다.<br>
      다시 선택 하세요! <br>
      <!-- 부모창의 id 값을 clear 하려면? -> only JS만 가능 -->
      <script>
      opner.document.getElementById('pid').value="";
      </script>
   </c:if>
</div>
